...
class Admins::<%= class_name.pluralize %>ControllerTest < ActionController::TestCase
  ...
  def test_get_new
    ...
    assert assigns(:<%= file_name %>)
    assert_template :new
    assert_select 'form[action=/admins/<%= file_name.pluralize %>]'
  end
  ...
  def test_get_edit
    ...
    assert_select "form[action=/admins/<%= file_name.pluralize %>/#{@<%= file_name %>.id}]"
  end
